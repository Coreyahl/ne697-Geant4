...
namespace ne697 {
  ...
  void Run::RecordEvent(G4Event const* event) {
    ...
    // we dont need to print this next line in the terminal when running many particles
    ...
    for (std::size_t ihit = 0; ihit < hc->entries(); ++ihit) {
      ...
      //do not print the hits in the terminal, comment out these G4cout (next 11 lines)
